...
class PdfSearchableExportService {
  ...
  /// to native PDF point coordinates (bottom-up ISO 32000-1 coordinate space).
  static ({
    double pdfX,
    double pdfY,
    double pdfWidth,
    ...
    final boxWidth =
        (clampedRect.right - clampedRect.left).clamp(0.0001, 1.0) * pageWidth;
    final boxHeight =
        (clampedRect.bottom - clampedRect.top).clamp(0.0001, 1.0) * pageHeight;

    final pdfX = lowerLeftX + (clampedRect.left * pageWidth);
    // In PDF bottom-left origin, Y = lowerLeftY + (1.0 - bottom) * pageHeight
    final pdfY = lowerLeftY + ((1.0 - clampedRect.bottom) * pageHeight);
  ...
  void _injectSearchableTextLayer(
    ...
    for (final word in sortedWords) {
      ...
      streamBuffer.writeln(
          '1 0 0 1 ${coords.pdfX.toStringAsFixed(2)} ${coords.pdfY.toStringAsFixed(2)} Tm');
